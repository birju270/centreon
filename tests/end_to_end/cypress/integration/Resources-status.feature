...
#   Given filter(s) already applied
...
#   Given the user has selected filter(s)
#   And the user has input a search pattern
#   When the user clicks on the SEARCH button
#   Then only resources matching the user selected filter(s) should be shown in the result
